...
try:
    ...
    try:
        ...
        if not firebase_admin._apps:
            ...
            if "FIREBASE_SERVICE_ACCOUNT_JSON_CONTENT" in st.secrets: # Check Streamlit secrets first
                try:
            ...
            if not cred_initialized and os.getenv("GOOGLE_APPLICATION_CREDENTIALS"): # Fallback to env var
                try:
            ...
            if not cred_initialized:
                print("INFO: No valid Firebase Admin credentials found. Using MOCK Firestore.")
...
def display_login_page():
    ...
    st.subheader("Login")
    with st.form("login_form_final"): # Using a consistent key
        email = st.text_input("Email", key="login_email_final")
        password = st.text_input("Password", type="password", key="login_password_final")
    ...
    if st.button("Don't have an account? Register here.", key="goto_register_btn_final"):
...
def display_register_page():
    ...
    with st.form("register_form_final"): 
        email = st.text_input("Email", key="register_email_final")
        password = st.text_input("Password", type="password", key="register_password_final")
        confirm_password = st.text_input("Confirm Password", type="password", key="register_confirm_password_final")
    ...
    if st.button("Already have an account? Login here.", key="goto_login_btn_final"):
...
def display_chat_app_page():
    ...
    if st.sidebar.button("Logout", key="logout_btn_chat_final"):
    ...
    with col_toggle_main:
        ai_model_toggled_on = st.toggle("Use AI Model (Gemini)", value=st.session_state.use_ai_model, key="ai_model_toggle_chat_final", help="Toggle ON for AI responses. Toggle OFF for local data lookup.")
    ...
    if not st.session_state.use_ai_model:
        ...
        with col_select_main:
            ...
            selected_display_option = st.selectbox("Select EDI Specification for Local Data:", options=display_options_list, index=display_options_list.index(current_display_value_for_selectbox), key="local_spec_selector_chat_final")
    ...
    with st.container(): 
        ...
        for i, msg_data in enumerate(st.session_state.messages):
            message(msg_data["content"], is_user=(msg_data["role"] == "user"), key=f"msg_chat_display_final_{i}", avatar_style="initials" if msg_data["role"] == "user" else "bottts", seed="User" if msg_data["role"] == "user" else "AI")
    st.markdown('</div>', unsafe_allow_html=True)
    st.markdown('<div class="sticky-input-area">', unsafe_allow_html=True)
    with st.form("user_input_form_chat_final", clear_on_submit=True): 
        user_input_value = st.text_input("Ask me anything!", key="user_input_widget_chat_final", placeholder="Ask me anything!", label_visibility="collapsed")
...
def main():
    ...
    load_css(os.path.join("assets", "style.css")) # Re-enabled CSS loading
